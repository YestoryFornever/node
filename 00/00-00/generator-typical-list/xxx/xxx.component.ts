...
@Component({
	...
	styleUrls: [
		'./<%= name %>.component.css',
		'../../../../../css/typical-list/header.css',
		'../../../../../css/typical-list/table.css',
		'../../../../../css/typical-list/condition.css',
	],
})
export class <%= upCaseName %>Component implements OnInit{
	...
	forAsyncTypeAhead:TypeAhead = new TypeAhead();
	asynForAsyn(){
		...
		}).mergeMap((token:string) => this.forAsyncTypeAhead.getStates(token));
	}
	...
	getList(){
		let param:any = {
			...
			orderBy:null,
			sortType:'ASC',
			forSync:this.condition.forSync,
